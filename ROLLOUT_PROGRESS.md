...
- [x] `ambient-focus-dashboard-kiosk` - Ambient Focus Dashboard and Kiosk Simulator (2026-06-14) - Two-state fullscreen productivity dashboard builder. State 1 (Configuration Studio): freeform WYSIWYG drag-and-drop canvas (16:9 aspect ratio preview) above a six-card control panel. Widgets: Clock and Calendar Module (digital/analog toggle, date format long/short/ISO, day-of-week toggle), Custom Text Label (auto-hides if blank), Target Countdown Banner (years/months/days/hours/minutes/seconds computed via Date delta, calendar-accurate), News and Financial Ticker (custom textarea items, marquee animation), Local Scratchpad (localStorage key axiomape_ambient_scratch, pointer-events preserved inside dashboard), Inspirational Image Carousel (FileReader API Data URI, user-defined rotation speed in seconds, interval-based cycling with opacity transition). Background gallery: OLED Black, four CSS gradients (Deep Space, Aurora, Midnight, Forest Night), and eight local image filenames (Persistence of Memory.jpg, Phobos.jpg, All Known Physics Equation.png, Ikigai.jpg, four screenshots). Freeform drag (mousedown delta tracking, percent-stored position) and CSS resize:both on all widgets. State 2 (Active Dashboard): Fullscreen API, site-chrome DOM elements hidden (axiom-header, axiom-footer, axiom-privacy-badge, axiom-disclaimer, ad wrappers, config-studio entirely hidden), zero ads visible. Exit logic: global keydown/click/touchstart listeners (capture phase) exit immediately via exitDashboard(); scratchpad textarea clicks do not exit. Analog clock drawn on canvas with requestAnimationFrame-compatible drawAnalogClock(). Ad placements strictly in State 1 only (axiom-ad-leaderboard after control panel, axiom-ad-mobile-anchor after main). 8-term glossary (Fullscreen API, Ambient Computing, Kiosk Mode, Local Storage, Image Data URI, WYSIWYG Canvas, Deep Work, Widget). 5-FAQ accordion matching JSON-LD FAQPage (uploaded images and server, exiting fullscreen, countdown calculation, ambient dashboards and deep work, ticker data source). applicationCategory: LifestyleApplication. Category: Home and Life. Dash check: 0.
